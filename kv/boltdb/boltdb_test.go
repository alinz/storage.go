...
import (
	...
	"github.com/alinz/storage.go/kv/boltdb"
)
...
func TestBoltdbStorage(t *testing.T) {
	...
	t.Run("Add a content", func(t *testing.T) {
		hashValue, n, err := bolt.Put(context.Background(), bytes.NewReader(content))
		assert.NoError(t, err)
	...
	t.Run("make sure the value exists", func(t *testing.T) {
		rc, err := bolt.Get(context.TODO(), expectedHashValue)
		assert.NoError(t, err)
		assert.NoError(t, tests.EqualReaders(rc, io.NopCloser(bytes.NewReader(content))))
	})

	t.Run("make sure only one item is return in List", func(t *testing.T) {
		next, cancel := bolt.List()
		defer cancel()
	...
	t.Run("make sure the deleted value no longer get accessed", func(t *testing.T) {
		err := bolt.Remove(context.TODO(), expectedHashValue)
		assert.NoError(t, err)
		_, err = bolt.Get(context.TODO(), expectedHashValue)
	...
	t.Run("put large values to the database", func(t *testing.T) {
		...
		rc, err := bolt.Get(context.TODO(), hashValue)
		assert.NoError(t, err)
